...
class _GrafikAktivitasScreenState extends State<GrafikAktivitasScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final double doublePesanan = totalPesanan.toDouble();
    final double doubleServis = totalServis.toDouble();
    final double doubleBarang = totalBarang.toDouble();
    final double doubleTotal = doublePesanan + doubleServis + doubleBarang;

    final double pctPesanan = doubleTotal > 0 ? (doublePesanan / doubleTotal) * 100 : 0.0;
    final double pctServis = doubleTotal > 0 ? (doubleServis / doubleTotal) * 100 : 0.0;
    final double pctBarang = doubleTotal > 0 ? (doubleBarang / doubleTotal) * 100 : 0.0;

    final double maxVal = [doublePesanan, doubleServis, doubleBarang].reduce((a, b) => a > b ? a : b);
    final double chartMaxY = maxVal > 30 ? maxVal + 10 : 40.0;
                          ...
                          SizedBox(
                            ...
                            child: BarChart(
                              BarChartData(
                                ...
                                barGroups: [
                                  BarChartGroupData(x: 0, barRods: [BarChartRodData(toY: doublePesanan, color: const Color(0xFF0284C7), width: 22, borderRadius: BorderRadius.circular(4))]),
                                  BarChartGroupData(x: 1, barRods: [BarChartRodData(toY: doubleBarang, color: const Color(0xFFD97706), width: 22, borderRadius: BorderRadius.circular(4))]),
                                  BarChartGroupData(x: 2, barRods: [BarChartRodData(toY: doubleServis, color: const Color(0xFF7C3AED), width: 22, borderRadius: BorderRadius.circular(4))]),
                                ],
                              ),
                            ),
                          ),
                                ...
                                child: SizedBox(
                                  ...
                                  child: PieChart(
                                    PieChartData(
                                      ...
                                      sections: doubleTotal == 0 
                                          ? [
                                              PieChartSectionData(color: Colors.grey.shade300, value: 1.0, title: '0%', radius: 25, titleStyle: const TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: Colors.white)),
                                            ]
                                          : [
                                              PieChartSectionData(color: const Color(0xFF0284C7), value: doublePesanan, title: '${pctPesanan.toStringAsFixed(0)}%', radius: 25, titleStyle: const TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: Colors.white)),
                                              PieChartSectionData(color: const Color(0xFFD97706), value: doubleBarang, title: '${pctBarang.toStringAsFixed(0)}%', radius: 25, titleStyle: const TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: Colors.white)),
                                              PieChartSectionData(color: const Color(0xFF7C3AED), value: doubleServis, title: '${pctServis.toStringAsFixed(0)}%', radius: 25, titleStyle: const TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: Colors.white)),
                                            ],
                                    ),
                                  ),
                                ),
